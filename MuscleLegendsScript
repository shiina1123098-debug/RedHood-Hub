...
local function CreateDropdown(parent, labelText, options, callback)
    ...
    header.Text = "Ã¢â€“Â¶  " .. labelText .. ": (ninguno)"
    ...
    for _, opt in ipairs(options) do
        ...
        ob.Activated:Connect(function()
            selected = opt
            header.Text = "Ã¢â€“Â¼  " .. labelText .. ": " .. opt
            open = false
            container.Size = UDim2.new(1, 0, 0, 34)
...
-- TAB: PLAYER
local playerPanel, playerTab = CreateTab("Player", "Ã°Å¸Å½Â®")
...
CreateLabel(playerPanel, "Velocidad / TamaÃƒÂ±o")
local speedVal = 120
CreateTextBox(playerPanel, "Speed (default 120)", function(v)
    local n = tonumber(v)
    if n and n > 0 then speedVal = n; Notify("Speed Ã¢â€ â€™ " .. n) end
end)
...
CreateTextBox(playerPanel, "Size (default 2)", function(v)
    local n = tonumber(v)
    if n and n > 0 then sizeVal = n; Notify("Size Ã¢â€ â€™ " .. n) end
end)
...
CreateToggle(playerPanel, "Speed Hack (+50)", function(v)
    ...
    if hum then
        ...
        Notify(v and "Speed Ã¢â€ â€™ 50" or "Speed Ã¢â€ â€™ 16 (normal)")
    end
end)
...
-- TAB: FARMING
local farmPanel, farmTab = CreateTab("Farming", "Ã°Å¸â€™Âª")

CreateLabel(farmPanel, "Fast Rep")
local repsPerTick = 1
CreateTextBox(farmPanel, "Rep Speed (default 1)", function(v)
    local n = tonumber(v)
    if n and n > 0 then repsPerTick = math.floor(n); Notify("Rep speed Ã¢â€ â€™ " .. repsPerTick) end
end)
...
CreateTextBox(farmPanel, "Target de Rebirths", function(v)
    local n = tonumber(v)
    if n then targetRebirths = n; Notify("Target Ã¢â€ â€™ " .. n) end
end)
...
-- TAB: FAST REBIRTH
local fastrebPanel, fastrebTab = CreateTab("Fast Reb", "Ã°Å¸â€â€ž")
...
-- TAB: KILLING
local killPanel, killTab = CreateTab("Killing", "Ã¢Å¡â€Ã¯Â¸Â")
...
CreateTextBox(killPanel, "Rango Aura (1-140)", function(v)
    local n = tonumber(v)
    if n then _G.deathRingRange = math.clamp(n, 1, 140); Notify("Rango Ã¢â€ â€™ " .. _G.deathRingRange) end
end)
...
-- TAB: PETS / SHOP
local petsPanel, petsTab = CreateTab("Pet Shop", "Ã°Å¸ÂÂ¾")
...
-- TAB: TELEPORTS
local tpPanel, tpTab = CreateTab("Teleports", "Ã°Å¸â€œÂ")
...
-- TAB: VISUAL
local visualPanel, visualTab = CreateTab("Visual", "Ã°Å¸â€˜ÂÃ¯Â¸Â")
...
CreateDropdown(visualPanel, "Change Time", {"Night","Day","Midnight"}, function(sel)
    ...
    Notify("Hora Ã¢â€ â€™ " .. sel)
end)

-- TAB: INVENTORY
local invPanel, invTab = CreateTab("Inventory", "Ã°Å¸Å½â€™")
...
-- TAB: MISC
local miscPanel, miscTab = CreateTab("Misc", "Ã¢Å¡â„¢Ã¯Â¸Â")
...
-- TAB: INFO
local infoPanel, infoTab = CreateTab("Info", "Ã¢â€žÂ¹Ã¯Â¸Â")
...
BubbleLabel.Text = "RH"
BubbleLabel.TextColor3 = Color3.fromRGB(255, 80, 80)
BubbleLabel.TextSize = 16
...
MinBtn.Activated:Connect(function()
    ...
    task.spawn(function()
        ...
        Tween(BubbleLabel, TweenInfo.new(0.3), {TextTransparency = 0})
    end)
end)
...
progressTween.Completed:Connect(function()
    ...
    Notify("RedHood Hub v4 - Full Edition listo! Ã¢Å“â€¦")
end)
