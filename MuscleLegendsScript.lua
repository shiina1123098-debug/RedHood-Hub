...
CreateToggle(fastrebPanel, "Auto Rebirth", function(v)
    ...
    local function doRebirth()
        ...
        local _goldenLevel = pcall(function() return player.ultimatesFolder["Golden Rebirth"].Value end) and player.ultimatesFolder["Golden Rebirth"].Value or 0
        local _discount = 1 - (_goldenLevel * 0.1)
        local strengthTarget = math.floor((10000 + (rebirthCount * 5000)) * _discount)
        while _G.AutoRebirthActive and player.leaderstats.Strength.Value < strengthTarget do
            waitForRespawn()
            local reps = player.MembershipType == Enum.MembershipType.Premium and 8 or 14
            for _ = 1, reps do muscleEvent:FireServer("rep") end
            task.wait(0.015)
        end
        if _G.AutoRebirthActive and player.leaderstats.Strength.Value >= strengthTarget then
            task.wait(0.2)
            local before = rebirths.Value
            repeat
                ReplicatedStorage.rEvents.rebirthRemote:InvokeServer("rebirthRequest")
                task.wait(0.1)
            until rebirths.Value > before or not _G.AutoRebirthActive
        end
    end
...
-- TextBox stat por golpe (guardamos referencia al tb para leerlo al activar)
local _, _tbStatPerHit = CreateTextBox(fastrebPanel, "Stat por golpe (default 5)", function(v)
    local n = tonumber(v)
    if n and n > 0 then _frRockStatPerHit = n; Notify("Stat/golpe -> " .. n) end
end)

-- TextBox target de pet stat
local _, _tbPetTarget = CreateTextBox(fastrebPanel, "Target Pet Stat total (ej: 850000)", function(v)
    local n = tonumber(v)
    if n and n > 0 then
        _frPetStatTarget = n
    ...
    task.spawn(function()
        while getgenv().FRRockRunning or frTrackerGui.Visible do
            ...
            if getgenv().FRRockRunning and _frPetStatTarget > 0 and totalStat >= _frPetStatTarget then
                -- Paso 1: detener Auto Rock limpiamente
                getgenv().FRRockRunning = false
                if _frTrackerConn then _frTrackerConn:Disconnect(); _frTrackerConn = nil end
                frTrackerGui.Visible = false
                Notify("Pet stat target alcanzado! Deteniendo Auto Rock...")
                task.wait(1.5)

                -- Paso 2: Auto King
                Notify("Activando Auto King...")
                ...
                -- Paso 5: Auto Rebirth
                Notify("Activando Auto Rebirth...")
                task.wait(2)
                _G.AutoRebirthActive = true
                task.spawn(function()
                    local function waitForRespawn()
                        if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then
                            player.CharacterAdded:Wait(); task.wait(0.7)
                        end
                        local hum = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
                        if hum and hum.Health <= 0 then
                            player.CharacterAdded:Wait(); task.wait(0.7)
                ...
                task.wait(0.5)
                Notify("Todo activado! King > Size > Weight > Rebirth")
                break
            end
            task.wait(0.5)
        end
...
CreateToggle(fastrebPanel, "Auto Rock (Pet Stat)", function(v)
    ...
    if v and _frRockSelectedRock then
        -- Leer textboxes aunque no se haya presionado Enter
        local spVal = tonumber(_tbStatPerHit and _tbStatPerHit.Text)
        if spVal and spVal > 0 then _frRockStatPerHit = spVal end
        local tgVal = tonumber(_tbPetTarget and _tbPetTarget.Text)
        if tgVal and tgVal > 0 then _frPetStatTarget = tgVal end
        frStartTracker()
        task.spawn(function()
